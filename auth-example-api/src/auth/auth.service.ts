...
export class AuthService {
  ...
  private async createSessionAndNotify(userId: string, ip: string, userAgent: string) {
    const sessionId = crypto.randomBytes(32).toString('hex');
    const sessionData = { userId, ip, userAgent, createdAt: new Date() };

    await this.redis.set(`session:${sessionId}`, JSON.stringify(sessionData), 'EX', 604800);
